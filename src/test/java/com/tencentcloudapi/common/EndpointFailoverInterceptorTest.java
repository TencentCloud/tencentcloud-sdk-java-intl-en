 ...
 * </ol>
 *
 * <h4>Polaris metadata-based failover</h4>
 * <ol start="52">
 *   <li>dfs type → *.internal.tencentcloudapi.com
 *   <li>internal type → *.{region}.tencentcloudapi.woa.com
 *   <li>mc type → *.{region}.api.tencentyun.com
 *   <li>trpc type → *.ai.tencentcloudapi.com
 *   <li>woa type → *.{region}.tencentcloudapi.woa.com
 *   <li>Missing region for types that need it → candidates=[urlHost], returns urlHost
 *   <li>No polaris tag → default TLD failover
 *   <li>Unknown type → candidates=[urlHost], returns urlHost
 *   <li>Nil/empty metadata → candidates=[urlHost], returns urlHost
 *   <li>Failover TLD breaker open → fallback to urlHost (Polaris instance host)
 * </ol>
 */
public class EndpointFailoverInterceptorTest {
    ...
    private static final Family N2 = new Family(
            ...
            "cvm.ai.tencentcloudapi.com",
            "cvm.ai.tencentcloudapi.com.cn",
            "cvm.ai.tencentcloudapi.cn");
    ...
    public void testIsKnownTencentCloudHost() {
        ...
        assertTrue(EndpointFailoverInterceptor.isKnownTencentCloudHost("cvm.ai.tencentcloudapi.com"));
        assertTrue(EndpointFailoverInterceptor.isKnownTencentCloudHost("cvm.ai.tencentcloudapi.cn"));
        assertTrue(EndpointFailoverInterceptor.isKnownTencentCloudHost("cvm.ai.tencentcloudapi.com.cn"));
        // Region-pinned.
        assertTrue(EndpointFailoverInterceptor.isKnownTencentCloudHost("cvm.ap-guangzhou.ai.tencentcloudapi.com"));
    ...
    public void testHostWithTldBuildsCorrectHosts() {
        ...
        assertEquals("cvm.ai.tencentcloudapi.com",
                EndpointFailoverInterceptor.hostWithTld("cvm.ai.tencentcloudapi.com", 0));
        assertEquals("cvm.ai.tencentcloudapi.com.cn",
                EndpointFailoverInterceptor.hostWithTld("cvm.ai.tencentcloudapi.com", 1));
    ...
    public void testMatchFamilyReturnsFirstMatchingFamily() {
        ...
        m = EndpointFailoverInterceptor.matchFamily("cvm.ai.tencentcloudapi.cn");
        assertNotNull(m);
        assertEquals("cvm", m.prefix);
    ...
    // intl prefix (cvm.intl.tencentcloudapi.com) maps to the normal family,
    // with "intl" folded into the prefix rather than treated as a TLD.
    @Test
    public void testMatchFamilyHandlesIntlPrefix() {
        EndpointFailoverInterceptor.Match m =
                EndpointFailoverInterceptor.matchFamily("cvm.intl.tencentcloudapi.com");
        assertNotNull(m);
        assertEquals("cvm.intl", m.prefix);
        assertEquals("tencentcloudapi.com", m.family[0]);
        assertEquals(0, m.tldIdx);
    }

    @Test
    public void testHostWithTldDropsIntlPrefix() {
        assertEquals("cvm.tencentcloudapi.com.cn",
                EndpointFailoverInterceptor.hostWithTld("cvm.intl.tencentcloudapi.com", 1));
        assertEquals("cvm.tencentcloudapi.cn",
                EndpointFailoverInterceptor.hostWithTld("cvm.intl.tencentcloudapi.com", 2));
    }

    // End-to-end: an intl-prefixed origin host rotates within the normal
    // family (.com -> .com.cn -> .cn), same as a plain public host.
    @Test
    public void testIntlHostFailoverRotatesWithinNormalFamily() throws Exception {
        Family f = new Family("Intl", "tencentcloudapi.com",
    ...
    public void testHostWithTldFromCnAndComCnOrigins() {
        // From .cn origin (tldIdx=2): idx 0 -> .com, idx 2 -> .cn
        assertEquals("cvm.tencentcloudapi.com",
                EndpointFailoverInterceptor.hostWithTld("cvm.tencentcloudapi.cn", 0));
        assertEquals("cvm.tencentcloudapi.cn",
                EndpointFailoverInterceptor.hostWithTld("cvm.tencentcloudapi.cn", 2));

        // From .com.cn origin (tldIdx=1): idx 0 -> .com, idx 1 -> .com.cn
        assertEquals("cvm.tencentcloudapi.com",
    ...
    @Test
    public void testHostWithTldDropsRegionInPrefix() {
        assertEquals("cvm.tencentcloudapi.com.cn",
                EndpointFailoverInterceptor.hostWithTld("cvm.ap-guangzhou.tencentcloudapi.com", 1));
        assertEquals("cvm.ai.tencentcloudapi.com.cn",
                EndpointFailoverInterceptor.hostWithTld("cvm.ap-guangzhou.ai.tencentcloudapi.com", 1));
    }
    ...
    public void testRegionPinnedHostIsEligibleForFailover() throws Exception {
        ...
        Family[] regionFamilies = {
            ...
            new Family("AI-R", "ai.tencentcloudapi.com",
                    "cvm.ap-guangzhou.ai.tencentcloudapi.com",
                    "cvm.ai.tencentcloudapi.com.cn",
                    "cvm.ai.tencentcloudapi.cn"),
    ...
    // =================================================================
    //  M14: Protocol-level failover (family loop)
    // =================================================================
    ...
    public void testAiFamilyRotationStaysWithinFamily() throws Exception {
        ...
        assertEquals(1, transport.received.size());
        assertEquals(f.firstFailover, transport.received.get(0).url().host());
        // Must be within ai.tencentcloudapi family, NOT plain cvm.tencentcloudapi.cn
    ...
    @Test
    public void testRegionPinnedHostFailoverDropsPrefix() throws Exception {
        ...
        assertEquals(1, transport.received.size());
        assertEquals(f.firstFailover, transport.received.get(0).url().host());
        assertFalse(transport.received.get(0).url().host().contains("ap-guangzhou"));
    }
    ...
    // =================================================================
    //  M23a: Candidate rotation follows family TLD order, not just the
    //  first candidate. Trip origin AND firstFailover's breakers, leaving
    //  only secondFailover closed — the request must land on secondFailover,
    //  proving buildCandidateHosts walks the family array in order
    //  ((tldIdx+1)%len, (tldIdx+2)%len, ...) rather than stopping/looping
    //  incorrectly after the first entry.
    // =================================================================

    @Test
    public void testFailoverAdvancesToSecondCandidateWhenFirstIsAlsoOpen() throws Exception {
        for (Family f : FAMILIES) {
            CvmClient client = newClient(f);
            tripBreakerFor(client, f.originHost, 60_000);
            tripBreakerFor(client, f.firstFailover, 60_000);
            TransportStub transport = installStub(client);
            transport.programOk();

            client.DescribeInstances(new DescribeInstancesRequest());
            assertEquals(f.name, 1, transport.received.size());
            assertEquals(f.name + ": origin and first candidate open, must advance to second",
                    f.secondFailover, transport.received.get(0).url().host());
        }
    }

    // Same idea, but verified directly against selectHost's returned
    // Candidate rather than through a full DescribeInstances round-trip —
    // pins down the exact rotation order independent of request signing.
    @Test
    public void testSelectHostWalksCandidatesInFamilyOrder() throws Exception {
        for (Family f : FAMILIES) {
            CvmClient client = newClient(f);
            EndpointFailoverInterceptor interceptor = failoverInterceptorOf(client);
            Request req = new Request.Builder()
                    .url("https://" + f.originHost + "/")
                    .header("Host", f.originHost)
                    .build();

            // All breakers closed: origin wins.
            assertEquals(f.name, f.originHost, interceptor.selectHost(req).host);

            // Origin open: first candidate (tldIdx+1) wins.
            tripBreakerFor(client, f.originHost, 60_000);
            assertEquals(f.name, f.firstFailover, interceptor.selectHost(req).host);

            // Origin + first open: second candidate (tldIdx+2) wins.
            tripBreakerFor(client, f.firstFailover, 60_000);
            assertEquals(f.name, f.secondFailover, interceptor.selectHost(req).host);

            // All three open: falls back to origin.
            tripBreakerFor(client, f.secondFailover, 60_000);
            assertEquals(f.name, f.originHost, interceptor.selectHost(req).host);
        }
    }
    ...
    public void testTc3ResignPreservesBodyAndContentType() throws Exception {
        for (Family f : FAMILIES) {
            ...
            Request resigned = transport.received.get(0);
            assertEquals(f.name, f.firstFailover, resigned.url().host());
            assertEquals(f.name, "application/json", resigned.header("Content-Type"));
