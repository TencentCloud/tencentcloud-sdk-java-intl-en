...
public class STSCredential extends Credential {
    ...
    private static class UpdaterImpl implements Credential.Updater {
        ...
        public void update(Credential credential) throws TencentCloudSDKException {
            STSCredential sts = (STSCredential) credential;
            if (sts.getSecretId() != null && !needRefresh()) {
                return;
            }
            updateCredential(sts);
        }

        private void updateCredential(STSCredential sts) throws TencentCloudSDKException {
