...
public class CvmRoleCredential extends Credential {
    ...
    private static class UpdaterImpl implements Credential.Updater {
        ...
        public void update(Credential credential) throws TencentCloudSDKException {
            CvmRoleCredential cvm = (CvmRoleCredential) credential;
            if (cvm.getSecretId() != null && !needRefresh()) {
                return;
            }
            updateCredential(cvm);
        }

        private void updateCredential(CvmRoleCredential cvm) throws TencentCloudSDKException {
            if (roleName == null) {
