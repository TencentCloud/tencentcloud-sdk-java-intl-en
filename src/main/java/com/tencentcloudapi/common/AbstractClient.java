...
public abstract class AbstractClient {
    ...
    public AbstractClient(
        ...
        // Polaris must run before failover: it rewrites URL.host to the Polaris instance host.
        this.trySetPolaris(this.httpConnection, profile.getPolarisProfile(), log);
    ...
    private void trySetPolaris(HttpConnection conn, PolarisProfile profile, TCLog log) {
        if (profile == null) {
            return;
        }
        conn.addInterceptors(new PolarisInterceptor(this, profile, log));
        // polaris internal cluster may have self-signed cert, so ignore CA verify
