...
Note: This field may return null, indicating that no valid values can be obtained.
     */
    public void setDataList(SimpleStringKeyValue [] DataList) {
     ...
     */
    public DescribeSimpleTeamListResp(DescribeSimpleTeamListResp source) {
        if (source.TotalCount != null) {
            this.TotalCount = new Long(source.TotalCount);
        }
        if (source.DataList != null) {
            this.DataList = new SimpleStringKeyValue[source.DataList.length];
            for (int i = 0; i < source.DataList.length; i++) {
                this.DataList[i] = new SimpleStringKeyValue(source.DataList[i]);
            }
        }
    }
