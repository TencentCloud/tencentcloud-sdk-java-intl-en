/*
 * Copyright (c) 2017-2025 Tencent. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.tencentcloudapi.csip.v20221121.models;

import com.tencentcloudapi.common.AbstractModel;
import com.tencentcloudapi.common.SSEResponseModel;
import com.google.gson.annotations.SerializedName;
import com.google.gson.annotations.Expose;
import java.util.HashMap;

public class KBFixSummaryItem extends AbstractModel {

    /**
    * KB patch ID
    */
    @SerializedName("KBId")
    @Expose
    private Long KBId;

    /**
    * KB patch name
    */
    @SerializedName("KBName")
    @Expose
    private String KBName;

    /**
    * KB No. (for example, KB5001234)
    */
    @SerializedName("KBNo")
    @Expose
    private String KBNo;

    /**
    * Number of related vulnerabilities
    */
    @SerializedName("RelatedVulCount")
    @Expose
    private Long RelatedVulCount;

    /**
    * Number of affected hosts
    */
    @SerializedName("AffectedCount")
    @Expose
    private Long AffectedCount;

    /**
    * Whether a system restart is required after the repair
    */
    @SerializedName("NeedReboot")
    @Expose
    private Boolean NeedReboot;

    /**
    * Prerequisite dependency patches (comma-separated list of KB numbers)
    */
    @SerializedName("KBPreCondition")
    @Expose
    private String KBPreCondition;

    /**
     * Get KB patch ID 
     * @return KBId KB patch ID
     */
    public Long getKBId() {
        return this.KBId;
    }

    /**
     * Set KB patch ID
     * @param KBId KB patch ID
     */
    public void setKBId(Long KBId) {
        this.KBId = KBId;
    }

    /**
     * Get KB patch name 
     * @return KBName KB patch name
     */
    public String getKBName() {
        return this.KBName;
    }

    /**
     * Set KB patch name
     * @param KBName KB patch name
     */
    public void setKBName(String KBName) {
        this.KBName = KBName;
    }

    /**
     * Get KB No. (for example, KB5001234) 
     * @return KBNo KB No. (for example, KB5001234)
     */
    public String getKBNo() {
        return this.KBNo;
    }

    /**
     * Set KB No. (for example, KB5001234)
     * @param KBNo KB No. (for example, KB5001234)
     */
    public void setKBNo(String KBNo) {
        this.KBNo = KBNo;
    }

    /**
     * Get Number of related vulnerabilities 
     * @return RelatedVulCount Number of related vulnerabilities
     */
    public Long getRelatedVulCount() {
        return this.RelatedVulCount;
    }

    /**
     * Set Number of related vulnerabilities
     * @param RelatedVulCount Number of related vulnerabilities
     */
    public void setRelatedVulCount(Long RelatedVulCount) {
        this.RelatedVulCount = RelatedVulCount;
    }

    /**
     * Get Number of affected hosts 
     * @return AffectedCount Number of affected hosts
     */
    public Long getAffectedCount() {
        return this.AffectedCount;
    }

    /**
     * Set Number of affected hosts
     * @param AffectedCount Number of affected hosts
     */
    public void setAffectedCount(Long AffectedCount) {
        this.AffectedCount = AffectedCount;
    }

    /**
     * Get Whether a system restart is required after the repair 
     * @return NeedReboot Whether a system restart is required after the repair
     */
    public Boolean getNeedReboot() {
        return this.NeedReboot;
    }

    /**
     * Set Whether a system restart is required after the repair
     * @param NeedReboot Whether a system restart is required after the repair
     */
    public void setNeedReboot(Boolean NeedReboot) {
        this.NeedReboot = NeedReboot;
    }

    /**
     * Get Prerequisite dependency patches (comma-separated list of KB numbers) 
     * @return KBPreCondition Prerequisite dependency patches (comma-separated list of KB numbers)
     */
    public String getKBPreCondition() {
        return this.KBPreCondition;
    }

    /**
     * Set Prerequisite dependency patches (comma-separated list of KB numbers)
     * @param KBPreCondition Prerequisite dependency patches (comma-separated list of KB numbers)
     */
    public void setKBPreCondition(String KBPreCondition) {
        this.KBPreCondition = KBPreCondition;
    }

    public KBFixSummaryItem() {
    }

    /**
     * NOTE: Any ambiguous key set via .set("AnyKey", "value") will be a shallow copy,
     *       and any explicit key, i.e Foo, set via .setFoo("value") will be a deep copy.
     */
    public KBFixSummaryItem(KBFixSummaryItem source) {
        if (source.KBId != null) {
            this.KBId = new Long(source.KBId);
        }
        if (source.KBName != null) {
            this.KBName = new String(source.KBName);
        }
        if (source.KBNo != null) {
            this.KBNo = new String(source.KBNo);
        }
        if (source.RelatedVulCount != null) {
            this.RelatedVulCount = new Long(source.RelatedVulCount);
        }
        if (source.AffectedCount != null) {
            this.AffectedCount = new Long(source.AffectedCount);
        }
        if (source.NeedReboot != null) {
            this.NeedReboot = new Boolean(source.NeedReboot);
        }
        if (source.KBPreCondition != null) {
            this.KBPreCondition = new String(source.KBPreCondition);
        }
    }


    /**
     * Internal implementation, normal users should not use it.
     */
    public void toMap(HashMap<String, String> map, String prefix) {
        this.setParamSimple(map, prefix + "KBId", this.KBId);
        this.setParamSimple(map, prefix + "KBName", this.KBName);
        this.setParamSimple(map, prefix + "KBNo", this.KBNo);
        this.setParamSimple(map, prefix + "RelatedVulCount", this.RelatedVulCount);
        this.setParamSimple(map, prefix + "AffectedCount", this.AffectedCount);
        this.setParamSimple(map, prefix + "NeedReboot", this.NeedReboot);
        this.setParamSimple(map, prefix + "KBPreCondition", this.KBPreCondition);

    }
}

