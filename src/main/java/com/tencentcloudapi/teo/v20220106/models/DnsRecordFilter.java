...
public class DnsRecordFilter extends AbstractModel {
...
- `status`: Site status.
    ...
    /**
     * Get Specifies whether to enable fuzzy query. It’s only available when the filter name is `name`. If it’s enabled, the length of `Values` must be 1. 
    ...
    /**
     * Set Specifies whether to enable fuzzy query. It’s only available when the filter name is `name`. If it’s enabled, the length of `Values` must be 1.
     ...
     */
    public DnsRecordFilter(DnsRecordFilter source) {
        if (source.Name != null) {
