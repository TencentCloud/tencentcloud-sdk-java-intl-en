...
Note: This field may return null, indicating that no valid values can be obtained.
     ...
     */
    public DescribeCmqQueuesResponse(DescribeCmqQueuesResponse source) {
        if (source.TotalCount != null) {
            this.TotalCount = new Long(source.TotalCount);
        }
        if (source.QueueList != null) {
            this.QueueList = new CmqQueue[source.QueueList.length];
            for (int i = 0; i < source.QueueList.length; i++) {
                this.QueueList[i] = new CmqQueue(source.QueueList[i]);
            }
        }
