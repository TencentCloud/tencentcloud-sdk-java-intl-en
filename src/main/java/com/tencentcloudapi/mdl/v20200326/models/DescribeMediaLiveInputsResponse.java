...
public class DescribeMediaLiveInputsResponse extends AbstractModel{
...
Note: this field may return null, indicating that no valid values can be obtained.
    ...
    public DescribeMediaLiveInputsResponse() {
    }
     ...
     */
    public DescribeMediaLiveInputsResponse(DescribeMediaLiveInputsResponse source) {
        if (source.Infos != null) {
            this.Infos = new InputInfo[source.Infos.length];
            for (int i = 0; i < source.Infos.length; i++) {
                this.Infos[i] = new InputInfo(source.Infos[i]);
            }
        }
